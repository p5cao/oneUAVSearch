...
local_pos_pub = rospy.Publisher('uav2/mavros/setpoint_position/local', PoseStamped, queue_size=1)

rospy.Subscriber('uav2/mavros/state', State, state_cb)
rospy.Subscriber('uav2/mavros/local_position/pose', PoseStamped, position_cb)
...
def position_control(list_of_setpoints0):
    ...
    rospy.init_node('offb_node2', anonymous=True)
    ...
    for i in range(num_of_points):
        ...
        while not rospy.is_shutdown() and not drone_reached(list_of_setpoints0[4*i] ,list_of_setpoints0[4*i+1],
            ...
            if prev_state.armed != current_state.armed:
                rospy.loginfo("UAV2 Vehicle armed: %r" % current_state.armed)
            if prev_state.mode != current_state.mode: 
                rospy.loginfo("UAV2 Current mode: %s" % current_state.mode)
            prev_state = current_state
...
if __name__ == '__main__':
	list_of_setpoints2 = read_data('/home/sza817353/catkin_ws/src/three_uav_wp/trial2_uav3_waypoints_ubd.txt')
	try:
        #print('D')
		position_control(list_of_setpoints2)
