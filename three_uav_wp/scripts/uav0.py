...
def position_control(list_of_setpoints0):
    ...
    print("uav0 waypoints are", list_of_setpoints0)
    ...
    for i in range(num_of_points):
        ...
        while not rospy.is_shutdown() and not drone_reached(list_of_setpoints0[4*i] ,list_of_setpoints0[4*i+1],
            ...
            if prev_state.armed != current_state.armed:
                rospy.loginfo("UAV0 Vehicle armed: %r" % current_state.armed)
            if prev_state.mode != current_state.mode: 
                rospy.loginfo("UAV0 Current mode: %s" % current_state.mode)
            prev_state = current_state
...
if __name__ == '__main__':
	list_of_setpoints0 = read_data('/home/sza817353/catkin_ws/src/three_uav_wp/trial2_uav1_waypoints_ubd.txt')
	try:
        #print('D')
		position_control(list_of_setpoints0)
